...
DO $$ BEGIN
    CREATE TYPE human_agent_status AS ENUM ('Offline', 'Available', 'Break', 'NotReady', 'InCall');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
    CREATE TYPE participant_type AS ENUM ('Customer', 'AiAgent', 'HumanAgent');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
    CREATE TYPE recording_status AS ENUM ('Pending', 'InProgress', 'Completed', 'Failed');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;

DO $$ BEGIN
    CREATE TYPE metric_type AS ENUM ('CallDuration', 'CallMinutes', 'TransferCount', 'RecordingMinutes', 'AgentSessionMinutes');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
    CREATE TYPE subscription_status AS ENUM ('Active', 'PastDue', 'Cancelled', 'Expired', 'Trialing');
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
VALUES
    ('a0000000-0000-0000-0000-000000000001', 'transfer_to_human', 'Transfer to Human',
     'Transfers the call to an available human agent within the same LiveKit room.',
...
CREATE TABLE IF NOT EXISTS call_participants (
    ...
    CONSTRAINT chk_call_participants_human_agent
        CHECK (
            (participant_type = 'HumanAgent' AND human_agent_id IS NOT NULL) OR
            (participant_type <> 'HumanAgent')
        )
);
...
CREATE TABLE IF NOT EXISTS plans (
    ...
    is_platform_plan    BOOLEAN NOT NULL DEFAULT TRUE,
    is_active           BOOLEAN NOT NULL DEFAULT TRUE,
...
CREATE INDEX IF NOT EXISTS ix_knowledge_bases_is_active ON knowledge_bases (is_active);

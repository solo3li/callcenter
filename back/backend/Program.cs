...
using (var scope = app.Services.CreateScope())
{
    ...
    // Run the SQL migration for enums, extensions, and full schema
    await RunSqlMigrationAsync(db, connectionString);

    // Reset all legacy agents to offline on startup
...
// ── MIGRATION RUNNER ──────────────────────────────────────────────────────────
async Task RunSqlMigrationAsync(AppDbContext db, string connectionString)
{
    var migrationSqlPath = Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "Migrations", "Sql", "001_initial_schema.sql");
    ...
    catch (PostgresException ex) when (ex.SqlState == "42710" || ex.SqlState == "42P07")
    {
